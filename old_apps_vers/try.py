...
def load_all_modalities(vnir_path, swir_path=None):
    ...
    red_idx, nir_idx = 29, 50  # Default AVIRIS fallback
    vnir_raw = None
    ...
    if ext == '.mat':
        ...
        if img_meta.bands.centers:
            ...
            if np.max(wl) < 100: wl = wl * 1000  # Convert um to nm if necessary
            red_idx = np.argmin(np.abs(wl - 650))
            nir_idx = np.argmin(np.abs(wl - 850))
            print(
                f"  ➤ Auto-Targeted NDVI Bands from Metadata: Red=Band {red_idx} ({wl[red_idx]:.1f}nm), NIR=Band {nir_idx} ({wl[nir_idx]:.1f}nm)")
        elif vnir_raw.shape[2] == 271:
            red_idx, nir_idx = 113, 203
            print(f"  ➤ Applied ADERET 271-band specific NDVI targeting: Red={red_idx}, NIR={nir_idx}")

    # Safety catch: prevent index out-of-bounds for arbitrary .mat files with fewer bands
    ...
    if not swir_path:
        print("  ➤ Running VNIR only (No SWIR provided).")
        # FIX: Explicitly pass vnir_clean as the 'fused_clean' fallback so it doesn't crash on None
...
def compute_semantic_weights(cube, vnir_raw, red_idx, nir_idx):
    """
    Calculates a continuous [0.0, 1.0] multiplier.
    Approaches 0.0 for confident nature (trees/soil).
    Approaches 1.0 for confident artifacts (metals/paints).
    """
    print("  ➤ Computing Soft Semantic Background Weights...")
    H, W, B = cube.shape

    # 1. Soft Vegetation Score (NDVI with exact bands)
    red_band = vnir_raw[:, :, red_idx]
    nir_band = vnir_raw[:, :, nir_idx]
    ndvi = (nir_band - red_band) / (nir_band + red_band + 1e-8)
    veg_score = np.clip((ndvi - 0.15) / 0.4, 0, 1)

    # 2. Soft Soil/Rock Score (PCA Residuals)
    X = cube.reshape(-1, B)
...
# ==========================================
# 3. Unsupervised Detectors
# ==========================================
def run_iforest(cube, n_components=10):
    print("  ➤ Running Isolation Forest (Continuous Scoring)...")
    H, W, B = cube.shape
    pca_features = PCA(n_components=n_components).fit_transform(cube.reshape(-1, B))
    ...
    iforest_map = scores.reshape(H, W)
    return (iforest_map - np.min(iforest_map)) / (np.max(iforest_map) - np.min(iforest_map) + 1e-8)
...
class InteractiveDashboard:
    def __init__(self, scene_img, heatmaps_dict, modality_name):
        ...
        # 2. Algorithm Panels (Synchronized zoom)
        idx = 1
        for name, heatmap in heatmaps_dict.items():
            ...
            slider = Slider(ax_slider, 'Thresh %', 80.0, 99.99, valinit=95.0, valstep=0.1)
        ...
        # 3. Final Target Detections (VOTING FUSION)
        ...
        self.ax_det.set_title("Robust Voting Detections (Cyan)", fontsize=12, fontweight='bold')
...
# ==========================================
# 5. Tri-Modal Execution Engine
# ==========================================
def execute_modality(modality_name, cube, rgb_background, semantic_weights, config):
    print(f"\n[{modality_name}] Initiating Evaluation...")
    active_heatmaps = {}

    # Run detectors on the pure, unmasked cube to preserve statistics
    if config['use_iforest']: active_heatmaps['IForest'] = run_iforest(cube)
    if config['use_osp']: active_heatmaps['OSP Unmixing'] = run_subpixel_unmixing_osp(cube, k_endmembers=15)
    if config['use_rx']: active_heatmaps['Local RX'] = run_local_rx_gpu(cube, inner_win=5, outer_win=15)

    if not active_heatmaps: return None

    # Post-Process: Multiply the output heatmaps by the Semantic Soft Weight
...
def run_pipeline(vnir_path, swir_path, config):
    print("\n" + "*" * 60)
    print("INITIALIZING SEMANTICALLY-AWARE HYPERSPECTRAL PIPELINE")
    print("*" * 60)
    ...
    # Calculate the Soft Weight Multiplier using the dynamic band indices
    # We pass fused_clean as the main argument if it exists, otherwise it correctly falls back to vnir_clean
...
if __name__ == "__main__":
    vnir_filepath = r"/abu-airport-1.mat"
    swir_filepath = None
